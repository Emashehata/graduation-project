...
export class HomeAdminComponent implements OnInit {
  ...
  // stats: Stats | null = null; // معرف كـ Stats أو null
  // upcomingAppointments: Appointment[] = []; // معرف كـ array من Appointment
  // latestComments: Comment[] = []; // معرف كـ array من Comment
  ...
  //   // التهيئة هتحصل هنا بعد ما الـ DOM يتحمل
  // }
  ...
  //       const labels = res.patientsByDay?.map((x: any) => x.day) || [];
  //       const data = res.patientsByDay?.map((x: any) => x.count) || [];

  //       // تهيئة الـ Donut Chart
  ...
  //               {
  //                 data: data,
  //                 backgroundColor: ['#0d6efd', '#36a2eb', '#66b0ff', '#99c7ff']
  //               }
  //             ]
  //           },
  //           options: {
  ...
  //     next: (res) => {
  //       const labels = res.map((x: any) => x.day) || [];
  //       const data = res.map((x: any) => x.count) || [];

  //       // تهيئة الـ Line Chart
  ...
  //       // ترتيب البيانات بناءً على ترتيب الأيام
  //       const sortedPatientsByDay = res.patientsByDay?.slice().sort((a, b) => {
  ...
  //           type: 'doughnut',
  //           data: {
  //             labels: labels, // الأيام دلوقتي مترتبة وبالعربي
  //             datasets: [
  //               {
  //                 data: data,
  //                 backgroundColor: ['#0d6efd', '#36a2eb', '#66b0ff', '#99c7ff']
  ...
  //       // ترتيب البيانات بناءً على ترتيب الأيام
  //       const sortedAppointmentsByDay = res.slice().sort((a, b) => {
  //         return this.dayOrder.indexOf(a.day) - this.dayOrder.indexOf(b.day);
  //       });
